...
def sphere(r=1.0, nSlices=24, nStacks=18):
	...
	for i in range(nStacks-2):
		for j in range(nSlices):
			t[nSlices+i*(2*nSlices)+2*j] = [1+i*(nSlices+1)+j, 1+i*(nSlices+1)+j+1, 1+(i+1)*(nSlices+1)+j]
			t[nSlices+i*(2*nSlices)+2*j+1] = [1+(i+1)*(nSlices+1)+j, 1+i*(nSlices+1)+j+1, 1+(i+1)*(nSlices+1)+j+1]
...
def subdivide(v, t):
	...
	for it in range(nt): # for every triangle
		...
		edgeLength = [np.linalg.norm(v[t[it][0]] - v[t[it][1]]), np.linalg.norm(v[t[it][1]] - v[t[it][2]]), np.linalg.norm(v[t[it][2]] - v[t[it][0]])]
